...
#   def slot_params
#     params.require(:slot).permit(:start_time, :end_time, :price,:status)
#   end
# end
...
class SlotsController < ApplicationController
  before_action :authenticate_user!
  before_action :set_branch_and_ground
  before_action :set_slot, only: %i[show edit update destroy]
  ...
  def destroy
    authorize @slot
    @slot.destroy
    redirect_to branch_ground_slots_path(@branch, @ground), notice: "Slot successfully deleted"
  end

  private

  def set_branch_and_ground
    @branch = Branch.find_by(id: params[:branch_id])
    @ground = @branch&.grounds&.find_by(id: params[:ground_id])
    return if @branch && @ground

    redirect_to branches_path, alert: "Branch or Ground not found"
  end

  def set_slot
    @slot = @ground&.slots&.find_by(id: params[:id])
    redirect_to branch_ground_path(@branch, @ground), alert: "Slot not found" unless @slot
  end
